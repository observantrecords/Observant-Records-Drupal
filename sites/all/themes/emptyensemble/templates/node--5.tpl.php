...
if (class_exists('OR_Albums')) {
	...
	$albums = $album_info->get_albums('empty-ensemble');
...
<ul class="album-list">
	<?php foreach ($albums as $album):
		$cover_url_base = OBSERVANTRECORDS_CDN_BASE_URI . '/artists/' . (!empty($artist_alias) ? $artist_alias : 'empty-ensemble') . '/albums/' . $album['album_alias'] . '/' . strtolower($album['release_catalog_num']) . '/images';
	?>
